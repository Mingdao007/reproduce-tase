...
| Implement tests before trusting plots | `scripts/run_tests.sh` used throughout; latest v143 validation: py_compile for the post-v142 completion gate passed; focused tests reported `4 passed in 0.48s`; full tests, YAML anchor scan, raw/heavy artifact scan, and `git diff --check` are pending final closeout; no live hardware commands were run | Done for current code |
