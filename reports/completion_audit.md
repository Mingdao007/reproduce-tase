...
|---|---|---|
| Use `Mingdao007/reproduce-tase` as target repo | `origin` is `git@github.com:Mingdao007/reproduce-tase.git`; decisions D001-D003; v37-v88 branches pushed and GitHub-verified; v89 is on local branch `exp/tase-ur10e-v89-readonly-run-audit` pending push verification | Done locally |
| Keep work Git-backed with dedicated branches | Iteration branches through `exp/tase-ur10e-v89-readonly-run-audit`; latest local branch is `exp/tase-ur10e-v89-readonly-run-audit` | Done locally |
...
| Preserve next-thread goal prompt | `docs/goal.md` and `docs/goal_handoff_v90.md` include the short prompt, authoritative local clone, v89 run-audit artifacts, claim boundary, and next executable read-only SOP execution/refinement target | Done locally |
| Maintain iteration log and decision record | `reports/ITERATION_LOG.md`, `reports/DECISION_RECORD.md`; decisions through D094 as of v89 | Done locally |
...
| Implement tests before trusting plots | `scripts/run_tests.sh` used throughout; latest v89 validation: `python3 -m py_compile scripts/audit_read_only_calibration_measurement_run.py scripts/create_read_only_calibration_measurement_run.py` passed, `scripts/run_tests.sh tests/test_read_only_calibration_measurement_template.py` reported `3 passed in 0.63s`, `scripts/run_tests.sh` reported `118 passed in 3.22s`, `git diff --check` passed, and no live hardware commands were run | Done for current code |
...
future evidence capture, and v89 adds an offline run-audit gate. The project
...
safe read-only portions of the v87 SOP with the v88 scaffold and v89 verifier
...
v38 trajectory-after-relaxed-setup, and v63-v89 diagnostic staged labels
