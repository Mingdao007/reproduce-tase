...
- Continue UR10e adapted work by executing only a safe read-only SOP subset
  ...
  simulation/paper-platform work identified by the v95-v106 blocker audits.
  The next offline probe can rerun the full `positive_fast_timing_0p0075`
  E1-E4 failed-cell audit with weighted priority while keeping the `0.12 rad`
  gate fixed, or switch to the `base_z_plus1mm` start-contact versus
