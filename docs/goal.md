...
- Continue UR10e adapted work by isolating the v73 `qdot012_stage_a18s`
  `+0.2 mm` Stage A final-tracking boundary with a small duration/path-retiming
  margin audit. Keep the faster-timing and tighter-orientation `+1.0 mm`
  failures as explicit sensitivity limits unless a separate model/control
  change is made, and keep strict paper-equivalent setup, v38
  trajectory-after-relaxed-setup, and v63-v73 diagnostic staged labels
  separate.
