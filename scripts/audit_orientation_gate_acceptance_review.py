...
FALSE_FIELDS = [
    ...
    ("review_execution", "user_approved_gate_acceptance_review"),
...
NULL_FIELDS = [
    ...
    ("orientation_gate_acceptance", "accepted_gate_type"),
    ("orientation_gate_acceptance", "accepted_gate_value_rad"),
    ("orientation_gate_acceptance", "accepted_normal_source"),
    ("orientation_gate_acceptance", "accepted_contact_datum_source"),
    ("orientation_gate_acceptance", "accepted_uncertainty_budget"),
    ("orientation_gate_acceptance", "reviewer"),
    ("orientation_gate_acceptance", "accepted_at_utc"),
]
...
def audit_review(review_dir: pathlib.Path) -> dict[str, Any]:
    ...
    if metrics_yaml.get("template_source") != "templates/orientation_gate_acceptance_review":
        violations.append("template_source does not point to the gate-acceptance review template")
    ...
    acceptance = metrics_yaml.get("orientation_gate_acceptance", {})
    ...
    if acceptance.get("review_only") is not True:
        violations.append("orientation_gate_acceptance.review_only is not true")

    review_path = review_dir / "gate_acceptance_review.md"
    if review_path.exists():
        review_text = review_path.read_text(encoding="utf-8")
        if "Decision status: `not_accepted`" not in review_text:
            violations.append("gate_acceptance_review.md does not preserve not_accepted decision status")
    ...
    return {
        ...
        "orientation_gate_acceptance": acceptance,
...
def write_summary(out_dir: pathlib.Path, payload: dict[str, Any]) -> None:
    lines = [
        "# Orientation Gate Acceptance Review Audit",
        "",
        f"Run root: `{out_dir}`",
        ...
        "## Claim Boundary",
        "",
        f"- Decision: `{payload['orientation_gate_acceptance'].get('decision')}`",
        f"- Review only: `{payload['orientation_gate_acceptance'].get('review_only')}`",
        f"- User approved gate acceptance review: `{payload['review_execution'].get('user_approved_gate_acceptance_review')}`",
...
def main() -> int:
    ...
    out_dir = (
        ...
        if args.output_dir
        else ROOT / "runs" / "orientation_gate_acceptance_review_audit" / run_id
    )
    if out_dir.exists():
