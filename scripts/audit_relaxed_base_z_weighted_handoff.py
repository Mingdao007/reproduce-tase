...
SCENARIOS: list[dict[str, Any]] = [
    {
        ...
        "description": "v70 relaxed-gate Stage B handoff baseline.",
    },
    {
        ...
        "orientation_kp": 0.0,
        "normal_axis_weight": 1.0,
        "description": "v106/v107 weighted zero-angular-command candidate.",
    },
    {
        ...
        "description": "v106/v107 weighted candidate with normal weight 30.",
    },
]
...
def duration_label(value: float) -> str:
    text = f"{float(value):.3f}".rstrip("0").rstrip(".").replace("-", "m").replace(".", "p")
    return f"{text}s"
...
def summarize_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "orientation_margin_to_gate_rad": None,
...
def summarize_case(
    *,
    ...
    for row in rows:
        row["orientation_margin_to_gate_rad"] = float(orientation_gate_rad) - row[
    ...
    return {
        ...
        "stage_a_qdot_saturation_fraction": float(
            metrics["stage_a"]["tracking"]["qdot_saturation_fraction"]
        ),
        "max_stage_b_orientation_error_rad": max(row["orientation_error_rad"] for row in rows),
        "min_stage_b_orientation_margin_to_gate_rad": min(
            row["orientation_margin_to_gate_rad"] for row in rows
        ),
...
def aggregate_cases(cases: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    durations = sorted({float(case["stage_a_duration_s"]) for case in cases})
    weighted_pass_by_duration = {
        float(duration): [
            case["scenario"]
            for case in weighted
            if float(case["stage_a_duration_s"]) == float(duration)
            and case["stage_a_passed"]
            and case["stitched_passed"]
        ]
    ...
    return {
        ...
        "tested_stage_a_durations_s": durations,
        ...
        "baseline_failing_count": sum(
            1 for case in baseline if not (case["stage_a_passed"] and case["stitched_passed"])
        ),
        "weighted_case_count": len(weighted),
        "weighted_passing_count": len(weighted_passing),
        "weighted_pass_by_duration": weighted_pass_by_duration,
        "weighted_candidate_recovered_any_duration": bool(weighted_passing),
        "weighted_candidate_recovered_all_durations": all(
            bool(weighted_pass_by_duration[duration]) for duration in durations
        ),
        "all_weighted_cases_recovered": len(weighted_passing) == len(weighted) and bool(weighted),
        "best_orientation_case": {
            "scenario": best_orientation["scenario"],
            "stage_a_duration_s": best_orientation["stage_a_duration_s"],
        },
        "min_max_stage_b_orientation_error_rad": best_orientation[
            "max_stage_b_orientation_error_rad"
        ],
        "max_stage_b_orientation_error_rad": max(
            case["max_stage_b_orientation_error_rad"] for case in cases
        ),
        "min_stage_b_orientation_margin_to_gate_rad": min(
            case["min_stage_b_orientation_margin_to_gate_rad"] for case in cases
        ),
        ...
        "candidate_closure_boundary": {
            "relaxed_handoff_candidate_recovered": bool(weighted_passing),
            ...
            "reason": (
                "This audit uses the run-local 0.12 rad relaxed gate and weighted "
                "Stage B priority as diagnostic candidates; neither is canonical."
            ),
        },
    }
...
def write_summary(out_dir: pathlib.Path, aggregate: dict[str, Any], cases: list[dict[str, Any]]) -> None:
    lines = [
        ...
        f"- Tested Stage A durations: `{', '.join(str(v) for v in aggregate['tested_stage_a_durations_s'])}`",
        f"- Passing cases: `{aggregate['passing_case_count']}`",
        f"- Weighted candidate recovered any duration: `{aggregate['weighted_candidate_recovered_any_duration']}`",
        f"- Weighted candidate recovered all durations: `{aggregate['weighted_candidate_recovered_all_durations']}`",
        f"- Original failed cell closed: `{aggregate['candidate_closure_boundary']['original_failed_cell_closed']}`",
        f"- Best orientation case: `{aggregate['best_orientation_case']['scenario']} @ {aggregate['best_orientation_case']['stage_a_duration_s']} s`",
    ...
    for case in cases:
        lines.append(
            "| `{duration}` | `{scenario}` | `{priority}` | `{normal}` | `{stitched}` | `{handoff}/{count}` | `{failed}` | `{orientation}` | `{qdot}` | `{tail_qdot}` | `{force}` |".format(
...
def main() -> int:
    ...
    durations = parse_float_list(args.stage_a_durations_s)
    run_id = dt.datetime.now().strftime("%Y%m%dT%H%M%S")
    out_dir = (
        pathlib.Path(args.output_dir)
        if args.output_dir
        else ROOT / "runs" / "relaxed_base_z_weighted_handoff" / run_id
    )
    ...
    for duration in durations:
        for scenario in SCENARIOS:
            ...
            case_dir = out_dir / "stage_a_durations" / duration_label(duration) / scenario_name
    ...
    payload = {
        ...
        "cases": cases,
        "claim_boundary": {
            "relaxed_base_z_handoff_probe": True,
            "weighted_candidate_recovered": bool(
                aggregate["weighted_candidate_recovered_any_duration"]
            ),
            "failed_cell_closed": False,
            "canonical_controller_change": False,
            "orientation_gate_relaxed_noncanonical": True,
        ...
        "warnings": [
            "diagnostic-label relaxed base_z_plus1mm handoff probe",
            "uses run-local 0.12 rad orientation gate from v70",
